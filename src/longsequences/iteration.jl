...
ambiguous_positions(seq::LongSequence) = AmbiguousNucleicAcidIterator(seq)
...
function find_next_ambiguous(seq::LongSequence{A}, i::Integer) where {A<:NucleicAcidAlphabet{2}}
    # no ambiguity
    return 0
end

function find_next_ambiguous(seq::LongSequence{A}, from::Integer) where {A<:NucleicAcidAlphabet{4}}
