@testset "Each k-mer" begin
    ...
    for k in [1, 3, 16, 32], step in 1:3, len in [1, 2, 3, 5, 10, 100, 1000]
        ...
        test_iteratorlength(MutableBioSequence{DNAAlphabet{2}}, random_dna(len, probs), k, step)
        test_iteratorlength(MutableBioSequence{RNAAlphabet{2}}, random_rna(len, probs), k, step)
    end
