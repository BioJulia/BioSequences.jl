...
Sequence views (`SeqView`s) are special, in that they do not own their own data,
and must be constructed from a `LongSequence` or another `SeqView`:
...
julia> typeof(seqview) == typeof(seqviev2) && typeof(seqview) <: SeqView
true
